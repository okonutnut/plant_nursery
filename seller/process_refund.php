...
if ($action !== 'approve' && $action !== 'reject') {
    header("Location: refunds.php?error=Invalid action");
    exit;
}
...
if (!$refund) {
    header("Location: refunds.php?error=Refund request not found");
    exit;
}

if ($refund['Status'] !== 'Pending') {
    header("Location: refunds.php?error=This refund request has already been processed");
    exit;
}
    ...
    if ($action === 'approve') {
        ...
        if ($item && $item['PlantID']) {
            ...
            if ($currentQuantity < $refundQuantity) {
                ...
                header("Location: refunds.php?error=Insufficient inventory for replacement. Only {$currentQuantity} units available for {$plantName}, but {$refundQuantity} units requested.");
                exit;
            }
            ...
            if (!$updateResult || mysqli_affected_rows($conn) == 0) {
                ...
                mysqli_query($conn, "UPDATE warranty_refund SET Status = 'Pending', ProcessedBy = NULL, ProcessedDate = NULL, Notes = NULL WHERE RefundID = $refundID");
                header("Location: refunds.php?error=Failed to update plant inventory. Please check stock availability.");
                exit;
            }
        }
    }
    
    header("Location: refunds.php?success=Refund request " . ($action === 'approve' ? 'approved and replacement processed' : 'rejected') . " successfully");
} else {
    header("Location: refunds.php?error=Error processing refund request: " . mysqli_error($conn));
}
exit;
